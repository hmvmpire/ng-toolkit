...
export default function index(options: any): Rule {
  ...
  function applyOtherNgToolkitSchematics(): Rule {
    return tree => {
      ...
      const workspace = getWorkspace(tree);
      //  `/${workspace.projects[options.clientProject].sourceRoot}`;


      // if (workspace.projects[options.clientProject].architect) {
    ...
    let rule: Rule = chain([
      ...
      externalSchematic('@nguniversal/express-engine', 'ng-add', expressOptions),
      (tree: Tree) => {
